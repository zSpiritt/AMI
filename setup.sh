...
if command -v apt &>/dev/null; then
    ...
    err "Gestionnaire de paquets non supporté. Installe les dépendances manuellement"
fi
...
BINARY=$(find "$TMP_DIR/src-tauri/target/release" -maxdepth 1 -type f -executable -name "ami" | head -1)
if [ -z "$BINARY" ]; then
    BINARY=$(find "$TMP_DIR/src-tauri/target/release" -maxdepth 1 -type f -executable | grep -v "\.d$\|\.rlib$\|build\|deps\|incremental" | head -1)
fi
...
if [[ ":$PATH:" != *":$BIN_DIR:"* ]]; then
    ...
    warn "Redémarre ton terminal pour utiliser la commande 'ami'"
fi
